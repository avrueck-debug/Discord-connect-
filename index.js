...
if (!DISCORD_BOT_TOKEN || !EDGE_URL || !BRIDGE_SECRET) {
  console.error("Missing env. Need DISCORD_BOT_TOKEN, EDGE_URL and BRIDGE_SECRET.");
  process.exit(1);
}
...
client.login(DISCORD_BOT_TOKEN).catch((err) => {
  ...
  if (/disallowed intents/i.test(msg)) {
    console.error(
      "👉 MESSAGE CONTENT INTENT is OFF. Enable it: Discord Developer Portal → your app → Bot → Privileged Gateway Intents → MESSAGE CONTENT INTENT ON → Save, then redeploy.",
    );
  }
  process.exit(1);
});
